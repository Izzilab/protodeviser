...
var PfamGraphic = Class.create( {
  ...
  setSequence: function( sequence ) {
    ...
    // Do not add +2 in the end, does not work nice with longer amino acid pixel size -petar
    //this._imageWidth = this._sequence.length * this._imageParams.residueWidth + 0;
  ...
  _buildMarkups: function() {
    ...
    orderedMarkups.each( function( markup ) {
      ...
      // do not use residueWidth, because overlapping lolipops will have an issue, with
      // certain values <0.5 and with anything over 1. Instead, fix it to the default 0.5...
      // or set the whole thing to to 2? This quick and dirt solution, however, prevents
      // the lollipops from stacking nicely.
      // -petar
      ...
      if ( h[ start - ( 1 / 0.5 ) ] !== undefined ||
           h[ start                        ] !== undefined ||
           h[ start + ( 1 / 0.5 ) ] !== undefined ) {
					   
        var firstLollipopHeight = h.slice( start - ( 1 / 0.5 ), 
                                           start + ( 1 / 0.5 ) ).max();
...
*/
  ...
  _drawRegion: function( region ) {
    ...
    // Make the width play nicely when using a custom residueWidth param -petar
    var height = Math.floor( this._regionHeight ) - 2,
        radius = Math.round( height / 2 ),
        arrow  = radius,
	//width  = ( region.end - region.start + 1 ) * this._imageParams.residueWidth,
        width  = ( region.end - region.start + 0 ) * this._imageParams.residueWidth,
  ...
  _drawMotif: function( motif ) {
    ...
    // Modify x to work better when specifying a custom residueWidth -petar
    var height = this._imageParams.motifHeight,
        //width  = Math.floor( ( motif.end - motif.start + 1 ) * this._imageParams.residueWidth ) + 1,
	width  = Math.floor( ( motif.end - motif.start + 0 ) * this._imageParams.residueWidth ) + 0,
        //x = Math.max( 1, Math.floor( ( motif.start + 1 ) * this._imageParams.residueWidth ) ),
	x = Math.max( 1, Math.floor( ( motif.start + 0 ) * this._imageParams.residueWidth ) ),
